...
void sigFpeHdlr(int signum){
	printf("%d thread with tid(%li) pid(%li) catched that %d thread divided by 0!!\n",findMyThreadId(),gettid(),(long)getpid(),threadsAmount-1);
	sleep(1);
}
...
void* threadFunc(void* arg){
	...
	while(1){
		printf("i am %d thread with tid(%li) pid(%li) lalala\n",myNum,gettid(),(long)getpid());
		sleep(1);
	}
...
int main(int argc, char **argv){
    ...
    if(argc<5){
        printf("four arguments: 1:threads amount, 2:file name, 3:amount of records read by thread \
in a single operation, 4:word to search\n");
        exit(1);
    }
	//init signal fpe
	signal(SIGFPE,sigFpeHdlr);
    //init values
